...
else if(cmd==='agents'){const rows=detectAvailableAdapters();for(const x of rows)console.log(`${x.available?'✓':'·'} ${x.id.padEnd(10)} ${x.label}${x.executable?`  (${x.executable})`:''}`);console.log('\nUse worker.adapter="auto" to select the first available agent, PHASE_AGENT=<id> to force one, or worker.manifest for any external harness.');}
