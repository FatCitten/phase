...
export function cosine(a,b){let d=0,aa=0,bb=0;for(let i=0;i<Math.min(a.length,b.length);i++){d+=a[i]*b[i];aa+=a[i]*a[i];bb+=b[i]*b[i];}return d/((Math.sqrt(aa)*Math.sqrt(bb))||1);}
