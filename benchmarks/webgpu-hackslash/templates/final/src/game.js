...
canvas.addEventListener('click',()=>{if(document.pointerLockElement!==canvas){canvas.requestPointerLock();return;} if(attack<=0.01){attack=1;for(const e of enemies){if(e.hp<=0)continue;const dx=e.x-player.x,dy=e.y-player.y,d=Math.hypot(dx,dy),da=Math.abs(wrap(Math.atan2(dy,dx)-player.a));if(d<1.75&&da<0.43){e.hp-=1;if(e.hp<=0)player.score++;}}}});
